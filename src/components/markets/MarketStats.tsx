      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-2 gap-y-1.5">
        {statsList.map((stat, index) => (
          <div key={index} className="flex items-center flex-row-reverse">
            {stat.icon}
            <span className="mr-1 text-gray-500">{stat.title}:</span>
            <span className="mr-1 font-medium">{stat.value}</span>
            {stat.change !== undefined && (
              <span
                className={`flex items-center mr-1 ${stat.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {stat.change >= 0 ? (
