...
export const MarketTable: React.FC<MarketTableProps> = ({ initialData }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
      <div className="p-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 flex-row-reverse">
        ...
        <Select value={sortBy} onValueChange={handleSort}>
          ...
          <SelectContent>
            <SelectItem value="market_cap_desc">قيمة السوق (من الأعلى للأدنى)</SelectItem>
            <SelectItem value="market_cap_asc">قيمة السوق (من الأدنى للأعلى)</SelectItem>
            <SelectItem value="volume_desc">الحجم (من الأعلى للأدنى)</SelectItem>
            <SelectItem value="volume_asc">الحجم (من الأدنى للأعلى)</SelectItem>
            <SelectItem value="price_desc">السعر (من الأعلى للأدنى)</SelectItem>
            <SelectItem value="price_asc">السعر (من الأدنى للأعلى)</SelectItem>
            <SelectItem value="percent_change_24h_desc">التغيير 24 ساعة (من الأعلى للأدنى)</SelectItem>
            <SelectItem value="percent_change_24h_asc">التغيير 24 ساعة (من الأدنى للأعلى)</SelectItem>
          </SelectContent>
        </Select>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">24 ساعة %</TableHead>
              <TableHead className="text-right md:table-cell hidden">7 أيام %</TableHead>
              <TableHead className="text-right lg:table-cell hidden">قيمة السوق</TableHead>
              <TableHead className="text-right xl:table-cell hidden">الحجم (24 ساعة)</TableHead>
              <TableHead className="text-right xl:table-cell hidden">المعروض المتداول</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {cryptoData.map((crypto: CryptoCurrency) => (
              <TableRow key={crypto.id} className="hover:bg-gray-50">
                ...
                <TableCell className="text-right">
                  <div
                    className={`inline-flex items-center flex-row-reverse ${crypto.percentChange24h >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <TableCell className="text-right md:table-cell hidden">
                  <div
                    className={`inline-flex items-center flex-row-reverse ${crypto.percentChange7d >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <TableCell className="text-right xl:table-cell hidden">
                  ...
                  <span className="text-gray-500 mr-1">{crypto.symbol}</span>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
