    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title,
          text: description || `اقرأ المقال: ${title}`,
          url,
        })
        setShareResult({ message: 'تمت المشاركة بنجاح!', type: 'success' })
        setIsOpen(false)
      } else {
        setShareResult({ message: 'المشاركة الأصلية غير مدعومة', type: 'error' })
      }
    } catch (error) {
      console.error('Error sharing content:', error)
      setShareResult({ message: 'فشلت المشاركة', type: 'error' })
    } finally {
      setIsSharing(false)
  ...
  const handleCopyLink = async (e: React.MouseEvent) => {
    ...
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        setShareResult({ message: 'تم نسخ الرابط!', type: 'success' })
      } else {
        try {
          const urlToCopy = url
          ...
          try {
            ...
            if (successful) {
              ...
              setShareResult({
                message: 'انسخ الرابط يدوياً',
                type: 'error',
              })
              setTimeout(() => {
                window.prompt('انسخ إلى الحافظة: Ctrl+C, Enter', urlToCopy)
              }, 500)
            }
          } catch (err) {
            console.error('Fallback copy method failed:', err)
            setShareResult({ message: 'انسخ الرابط يدوياً', type: 'error' })
            setTimeout(() => {
              window.prompt('انسخ إلى الحافظة: Ctrl+C, Enter', urlToCopy)
            }, 500)
          } finally {
          ...
          setShareResult({
            message: 'يرجى نسخ الرابط من شريط العنوان',
            type: 'error',
          })
        }
      }
      ...
      setShareResult({ message: 'فشلت عملية نسخ الرابط', type: 'error' })
    } finally {
      setIsSharing(false)
      setTimeout(() => setShareResult(null), 3000)
    }
          ...
          <div className="flex flex-wrap gap-2 justify-between">
            {shareLinks.map((socialLink) => (
              <a
                ...
                aria-label={`شارك على ${socialLink.name}`}
              >
                {socialLink.icon}
              </a>
            ))}
