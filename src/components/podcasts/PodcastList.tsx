      ...
      <div className="container mx-auto w-full px-4 py-6">
        ...
        <div className="mb-8">
          ...
          <div className="lg:hidden">
            ...
            {/* Search Bar below */}
            {showSearch && (
              <form onSubmit={handleSearchSubmit} className="flex items-center gap-2 flex-row-reverse">
                <div className="relative flex-1">
                  <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 w-4 h-4" />
                  <Input
                    type="text"
                    placeholder="ابحث في البودكاستات..."
                    value={searchInput}
                    onChange={(e) => setSearchInput(e.target.value)}
                    className="pr-10 pl-10 h-10 border-slate-200 focus:border-primary focus:ring-primary/80 rounded-lg text-right"
                  />
                  {searchInput && (
                    <button
                      type="button"
                      onClick={handleClearSearch}
                      className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-primary transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  )}
                ...
                <Button
                  type="submit"
                  className="h-10 px-3 bg-primary hover:bg-primary/90 text-white rounded-lg"
                >
                ...
                <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      value={searchInput}
                      onChange={(e) => setSearchInput(e.target.value)}
                      className="pr-10 pl-10 h-10 w-64 border-slate-200 focus:border-primary focus:ring-primary/80 rounded-lg text-right"
                    />
                    {searchInput && (
                      <button
                        type="button"
