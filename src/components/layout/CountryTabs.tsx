            ...
            {countries.map((country) => {
              ...
              return (
                <button
                  ...
                  className={`group relative shrink-0 px-1 ${labelSize} whitespace-nowrap transition-colors ${
                    isLoading ? 'opacity-60 cursor-wait' : 'cursor-pointer'
                  } ${isActive ? 'text-primary font-semibold' : 'text-gray-600 hover:text-secondary'} `}
                >
                  ...
                  <span
                    className={`pointer-events-none absolute left-1/2 -translate-x-1/2 bottom-0 h-0.5 rounded-full transition-all duration-300 ${
                      isActive
                        ? 'w-full bg-primary'
                        : 'w-0 bg-transparent group-hover:w-full group-hover:bg-muted'
                    }`}
                  />
                  {isLoading && (
                    <BiLoaderAlt className="absolute -top-1 -right-3 h-3 w-3 animate-spin text-primary" />
                  )}
                </button>
              )
            })}
