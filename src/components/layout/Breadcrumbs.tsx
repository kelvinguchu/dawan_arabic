...
const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ customItems, className = '' }) => {
  ...
  const generateBreadcrumbs = (): BreadcrumbItem[] => {
    ...
    const breadcrumbs: BreadcrumbItem[] = [{ name: 'الصفحة الرئيسية', href: '/' }]
    ...
    pathSegments.forEach((segment) => {
      ...
      if (segment === 'news') name = 'آخر الأخبار'
      if (segment === 'category' || segment === 'categories') name = 'الأقسام'
      if (segment === 'about') name = 'من نحن'
          ...
          <ol className="flex items-center space-x-2 text-sm flex-row-reverse">
            {breadcrumbs.map((item, index) => (
              <li key={item.href} className="flex items-center">
                ...
                {index === 0 && <BiHome className="w-4 h-4 text-gray-500 ml-1" />}
