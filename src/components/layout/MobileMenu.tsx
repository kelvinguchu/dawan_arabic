  ...
  if (authLoading) {
    ...
    authSectionContent = (
      <>
        <Link
          href="/account"
          className="flex items-center px-3 py-2 rounded-md text-gray-700 hover:bg-gray-50 hover:text-primary flex-row-reverse"
          onClick={() => setIsMenuOpen(false)}
        >
          <Avatar className="h-6 w-6 ml-2 border border-slate-200">
            <AvatarImage
              src={
                user.profilePicture &&
        ...
        >
          <BiLogOut className="h-5 w-5 ml-2" />
          تسجيل الخروج
        </button>
      </>
    )
    ...
    authSectionContent = (
      <>
        <Link
          href="/login"
          className="block px-3 py-2 rounded-md text-gray-700 hover:bg-gray-50 hover:text-primary"
          onClick={() => setIsMenuOpen(false)}
        >
          تسجيل الدخول
        </Link>
            ...
            {isInstallable && (
              <div className="px-3 py-2">
                <button
                  ...
                  className="flex items-center gap-2 w-full px-4 py-3 bg-primary text-white rounded-md hover:bg-primary/80 transition-colors"
                >
                  <BiDownload className="h-5 w-5" />
                  <span className="font-medium">ثبت تطبيق بوابة أفريقيا</span>
                </button>
              </div>
            )}
          </div>
