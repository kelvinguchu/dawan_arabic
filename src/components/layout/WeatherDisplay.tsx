...
const WeatherDisplay: React.FC<WeatherDisplayProps> = ({ isMobile = false }) => {
  ...
  const getWeatherCondition = (code: number) => {
    ...
    if (code >= 51 && code <= 55) return 'مطر خفيف'
    if (code >= 56 && code <= 57) return 'مطر خفيف مجمد'
    if (code >= 61 && code <= 65) return 'مطر'
    if (code >= 66 && code <= 67) return 'مطر مجمد'
      ...
      <div className="flex items-center text-xs text-gray-500">
        {weather.loading ? (
          ...
          <span className="flex items-center">
            <BiCloud className="ml-1.5 h-4 w-4" />
            لا يمكن الحصول على الطقس
          </span>
        ) : (
  ...
  return (
    <div className="hidden md:flex items-center text-xs text-gray-500">
      {weather.loading ? (
        ...
        <span className="flex items-center">
          <BiCloud className="ml-1.5 h-3.5 w-3.5" />
          لا يمكن الحصول على الطقس
        </span>
      ) : (
        <div className="flex items-center">
