...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const forgotPassword = useCallback(async (email: string): Promise<ForgotPasswordResult> => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'فشل إرسال بريد إعادة تعيين كلمة المرور'
  ...
  const resendVerification = useCallback(
    async (email: string): Promise<ResendVerificationResult> => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'فشل إعادة إرسال بريد التحقق'
        setError(errorMessage)
        return {
          success: false,
          error: errorMessage,
